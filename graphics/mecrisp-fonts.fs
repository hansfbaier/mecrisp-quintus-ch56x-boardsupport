...
\ bitmap fonts for use with mecrisp
\ These are monospaces fonts
...
\ I've taken the fonts from Matthias's
\ common/graphics-unicode-8x8.txt and
\ common/graphics-unicode-16x8.txt
\ I also convertet bitmap fonts I found in /usr/share/consolefonts/
\ under the use of gforth PSF.fs
\ bash commands for doing this:
\  gforth psf.fs [source-path/file] [destination-path]
\ or to convert all fonts in a directory
\ for font in /usr/share/consolefonts/*psf* ;
\ ; do gforth src/psf.fs $font mecrispfont ;
\ done
...
8x8font Variable font
8 Variable font-width
8 Variable font-height

: glyphsize ( -- n )
    font-width @ 8 /mod swap IF 1 + THEN
    font-height @ *
    2 +
;
